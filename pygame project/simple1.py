import pygame
from pygame.locals import *
import random

size = width, height = (800, 800)

# Road width
road_w = int(width/1.6)

# Roadmark width
roadmark_w = int(width/80)

# Right lane
right_lane = width/2 + road_w/4

# Left lane
left_lane = width/2 - road_w/4

# Speed
speed = 1

# Initialise game
pygame.init()

running = True

# Sets the screen size
screen = pygame.display.set_mode(size)

# Sets the window title
pygame.display.set_caption("The car game")

# Sets the screen background colour
screen.fill((60, 220, 0))

# Applies changes / Refreshes screen
pygame.display.update()

# Load player car
car = pygame.image.load("white car.png")
car_loc = car.get_rect()
car_loc.center = left_lane, height*0.8

# Load enemy car
car2 = pygame.image.load("red car.png")
car2_loc = car.get_rect()
car2_loc.center = right_lane, height*0.2

# Counter
counter = 0

# Game loop
while running:
    counter += 1
    if counter == 1024:
        speed += 0.15
        counter = 0
        print("level up", speed)

    # Enemy car animation
    car2_loc[1] += speed
    if car2_loc[1] > height:
        if random.randint(0,1) == 0:
            car2_loc.center = right_lane, -200
        else:
            car2_loc.center = left_lane, -200

    # Events
    for event in pygame.event.get():
        if event.type == QUIT:
            running = False
        if event.type == KEYDOWN:
            if event.key in [K_a, K_LEFT]:
                car_loc = car_loc.move([-int(road_w/2), 0])
            if event.key in [K_d, K_RIGHT]:
                car_loc = car_loc.move([int(road_w/2), 0])

    # end game
    if car_loc[0] == car2_loc[0] and car2_loc[1] > car_loc[1] - 225:
        print("!! GAME OVER !! YOU LOSE !!")
        break

    # Draws graphics
    # Road
    pygame.draw.rect(
        screen,
        (50, 50, 50),
        (width/2-road_w/2, 0, road_w, height)
        )

    # Centre markings
    pygame.draw.rect(
        screen, 
        (255, 240, 60),
        (width/2 - roadmark_w/2, 0, roadmark_w, height)
        )

    # Side markings
    pygame.draw.rect(
        screen, 
        (255, 255, 255),
        (width/2 - road_w/2 + roadmark_w*2, 0, roadmark_w, height)
        )

    pygame.draw.rect(
        screen, 
        (255, 255, 255),
        (width/2 + road_w/2 - roadmark_w*3, 0, roadmark_w, height)
        )


    screen.blit(car, car_loc)
    screen.blit(car2, car2_loc)
    pygame.display.update()

# Quits
pygame.quit()